...
3. Deploy. No secrets are required — the LLM API key (Anthropic or Google AI Studio) is entered by the user in the sidebar at runtime and is never stored.
...
3. **Query Fan-out Generator tab** — in the sidebar, choose your LLM provider (**Anthropic** at [console.anthropic.com](https://console.anthropic.com) or **Google AI Studio** at [aistudio.google.com/apikey](https://aistudio.google.com/apikey)), enter the corresponding API key, pick a model, optionally add business context (this makes entity expansions domain-specific), then click *Generate Fan-outs*.
